...
class shopDeliveryshopPluginHelper 
{
	...
	public function __construct($CIDRFile = false, $CitiesFile = false)
	{
		if(!$CIDRFile)
		{
			$CIDRFile = wa()->getAppPath("/plugins/deliveryshop/files/").'cidr_optim.txt';		
		}
		if(!$CitiesFile)
		{
			$CitiesFile = wa()->getAppPath("/plugins/deliveryshop/files/").'cities.txt';			
		}
    ...
    public function getCitySearch($query)
	{
		...
		while(!feof($this->fhandleCities))
		{
            ...
            if (strtolower($arRecord[1]) == strtolower($query))
            {
                $result = $arRecord;
            }	
		}
